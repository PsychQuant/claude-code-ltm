...
/// 巧合而是 RRF 的結構性質。
///
/// **發生率已量**（#18，`docs/measurements/2026-08-22-rrf-tie-rate.md`）：不低，
/// 但幾乎完全是一個 `cjk-2char` 現象。在 16,959 chunk 的索引上，中文雙字查詢的
/// **每一個** top-20 候選都落在平手段內（相鄰平手 52.6%，該桶的結構上限）；
/// `cjk-4plus` 與 `mixed` 則一次都沒有觀測到平手——在那兩桶裡這一檔逐字等同
/// `archival`。
///
/// 機制比上面那段推導更窄：平手需要**兩個**通道各自貢獻獨佔候選並在名次上對撞。
/// 中文雙字查詢的 `trigram` 通道整條空掉（3 字元硬底線，見
/// `docs/measurements/2026-08-10-fts5-tokenizer.md`），只剩 `segment` 與 `vector`
/// 兩組互斥的單通道候選成對同分——這也是為什麼實測的平手段長度**恆為 2**。
///
/// **誠實邊界（仍然成立的部分）**：量到的是「有沒有可動的空間」，不是「動得對
/// 不對」。這一檔是否**優於** `archival` 或 `human-like` 仍然沒有根據——那需要
/// `(查詢, 應命中的 turn)` 的評估集（#16）。
///
/// ## 一個誠實的脆弱處
///
