...
/// 唯一寫在這裡的結論是**定性**的：這一檔有實際作用面，而那個作用面高度集中在
/// `cjk-2char`。**所有數字只住在那份紀錄裡**，這裡不複述——連「某個桶是 0」都不
/// 複述。上一版在這裡寫了兩個桶的零觀測，而同一個 commit 落地的量測表就已經
/// 否證了其中一個（#18 verify R2）：撤回寫進了**描述**撤回的文件，沒寫進**被**
/// 撤回的文件。不複述數字是唯一能結構性避免這件事重演的寫法。
///
...
///
/// 1. **不得從任何被觀測到的段長分佈推導上限。** 這是一條性質，不繫於哪一次觀測
///    到什麼——平手**不限於**單通道候選：`rrfK = 60`、通道深度 100 時，
///    `1/(60+36)+1/(60+100)`、`1/(60+40)+1/(60+90)`、`1/(60+60)+1/(60+60)` 是
///    **同一個有理數**，所以多個雙通道候選可以逐位元同分而完全不需要 `trigram`。
///    （上一版在這裡引用了「觀測到的段長都是 2」——而那個觀測本身在同一次重跑裡
///    就被推翻了。一條警告「別從觀測推上限」的條文，自己引用了一個錯的觀測。）
/// 2. **不得把「某個桶沒觀測到平手」寫成任何形式的斷言。** 桶內只有 20 條人工
///    查詢，零觀測既撐不起「等同 `archival`」，也撐不起「未觀測到」本身——後者
///    在換一個抽樣後就變成假的。
