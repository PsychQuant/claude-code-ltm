...
public final class IndexDatabase {
    ...
    public func createSchema() throws {
        ...
        // 不是崩潰特徵），而在現有持久化資訊下**不存在完整、sound、又不進熱路徑
        // 的偵測**：parse 前綴數 chunk 落在 #49 刪掉的熱路徑上（且對 skip-only
        // 區段仍證明不了什麼）；序數落地要改 schema（#47 declined）；便宜的
        // 「cursor > 0 而 chunk 數 == 0」heuristic 本身不 sound——對純 skip 行的
        // 合法檔案偽陽性、會造成 #47 F1 剛修掉的永久重失效迴圈。（措辭經跨模型
        // 驗證收斂：codex 以雙歷史構造證明兩種來歷可產生逐 byte 相同的現狀，
        // 任何只看現狀的檢查原理上不可區分。）改 DB 游標需要的權限與直接刪 chunk
