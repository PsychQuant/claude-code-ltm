...
> **#63 註記（2026-09-07；2026-09-10 更正）**：本紀錄的 SQL 表（Q1／Q2）與 `ltm build` 不涉文字查詢、不受基準查詢污染影響。但下方 A/B 表的 **`query ×3` 欄**與結論裡的「1 秒以內」是端對端 `ltm query`——**查詢字串沒有記錄**，量測在污染 turn（臺北 12:32）之後，是否已被污染無從判斷；耗時仍可比、命中本來就沒記。第一版寫「沒有文字查詢、不受影響」，被 #63 verify R10 對著這張表證偽。同形的註記在 `2026-09-01-scan-parallelism.md`。
