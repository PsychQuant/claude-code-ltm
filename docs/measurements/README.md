...
- 查詢檔本身是一般 tracked blob、**明文在 GitHub 伺服器上**；`-diff` 只擋 diff 生成，規則 1 又禁止 review agent
  ...
  紀錄可查；`rrf-tie-queries.txt` 同樣 tracked 且 `-diff`。自動內容約束是封閉的四條（測試 `checkQueryFile`，`maxQueryScalars` 的說明是同一份；R23 統一——R22 版這裡與
  測試各寫一個互斥的「唯一」）：退役查詢的子字串比對（空白摺疊＋小寫）、不得重複、禁用字元（控制字元、非 ASCII 空白、BOM，
  含註解行）、非註解行的每條純量上限（註解行沒有長度上限、條數只有下限）。它們擋不住整段文字（切成多條短行、或放進 `#`
