...
- 查詢檔本身是一般 tracked blob、**明文在 GitHub 伺服器上**；`-diff` 只擋 diff 生成，規則 1 又禁止 review agent
  ...
  紀錄可查；`rrf-tie-queries.txt` 同樣 tracked 且 `-diff`。內容約束只有測試對**非註解行**的每條純量上限（`maxQueryScalars`；註解行沒有長度上限、條數只有下限）——它擋不住
  整段文字：切成多條短行、或放進 `#` 註解都過（codex R22 證偽了 R21 版的「擋的是整段文字的量級」），更分不出一句第三方
  逐字短句與自行撰寫的短語；作者自審是這個檔的防線。
