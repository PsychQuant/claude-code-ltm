...
1. **查詢字串不得落進會被索引的欄位。** 具體（由上表導出，不是另一份清單；表改了這裡要跟）：
   ...
   **`grep '^#'` 的安全性有確切的射程，而且只對 `baseline-queries.txt` 成立**：「檔頭結構」那一條自動內容約束
   （終止符恰好一行、之前只有註解、之後沒有註解）保證**沒有任何一行查詢能靠加 `#` 退役**——R26 版把約束綁在
   「第一條查詢之後」這個位置上，把**第一條**查詢註解掉時那一行會升格成檔頭而全綠（R27 四家實測），所以 R27
   換成與位置無關的終止符。它**不**保證檔頭區塊的內容——檔頭裡放什麼完全不受約束，`grep '^#'` 印出來的就是作者
   放進檔頭的東西（R27 實測：檔頭放一行含查詢的註解，八條全過）。所以**退役一條查詢必須刪除**；把它搬進檔頭仍然
   印得出來，那是人守的。`rrf-tie-queries.txt` **一條內容約束都沒有**（`checkQueryFile` 只跑在 baseline 上，全 repo
   唯一碰它的測試是 `git check-attr`），對它 `grep '^#'` 的安全性零機制——它的處置追蹤於 #68。
   清「repo 之外的副本」的判準是**內容**，鍵不能是檔名、也不能只靠地點：R25 用兩個地點的列舉漏掉 `~/.claude/jobs/**`
   七棵 reviewer 樹；R26 改成四個根目錄＋`-name "$(basename $f)"`——**還是檔名**，而 `~/.claude/file-history/` 裡三份
   逐位元組相同的明文副本檔名是雜湊＋`@vN`，那條掃描結構上看不到（R27，security；已清）。查法：對每個受限檔，
   以 **size 篩候選、`cmp -s` 定案**——`find <根目錄…> -type f -size "$(stat -f%z $f)c" | while read c; do cmp -s "$c" "$f"
   && echo COPY "$c"; done`，根目錄至少含 `~/.claude`（含 `file-history`、`jobs`）、`/private/tmp`、`/private/var/folders`
   （`/tmp` 是 symlink，`find` 不跟隨，列了等於沒列）。**有限根目錄證不出「零副本」**，只證得出「這幾棵樹裡零份」——
   報告要寫後者。命中的 checkout **不要靜默排除，要另列**：reviewer 的私有 worktree 就是「remote 指向本 repo 的
   checkout」，R26 的排除子句把它要抓的那一類整個排掉（R27，security）；今天已知且接受的一份是 marketplace 的 clone
   （`~/.claude/plugins/marketplaces/claude-code-ltm/`），其餘一律回報。
...
- **規則在某個條目長度以下不可達成**——這一條決定的不是「有沒有漏洞」，是「這條規則對哪一半的受保護內容
  ...
  要建的那組儀器立得起來。**但「零命中」是逐字比對的結果，它的邊際很薄**：8 條裡有幾條只差兩個字元就是工作樹裡
  現成的字串，而那些近似字串已經在被索引的語料裡出現幾十次；judge 的 `self` 判定是整條的子字串包含，這類近似殘影
  它**看不見**、一律判 `clean`（同規則 3 那段寫的「clean 不是乾淨的證明」——這句結論不例外，R27，security）；而 sister set（`rrf-tie-queries.txt`）**超過一半**的條目逐字出現在工作樹裡，其中一批
  來自本 repo 自己的中文註解與 commit 文字，最短的條目只有兩個字元。規則 3 的 `self` 分診也救不了它：judge 的
  判定是子字串包含，所以那些命中每一個都會判成 `self`，而規則 3 的預設處置（退役）會退掉超過一半的集合。
...
- 查詢檔本身是一般 tracked blob、**明文在 GitHub 伺服器上**；`-diff` 只擋 diff 生成，規則 1 又禁止 review agent
  ...
  紀錄可查；`rrf-tie-queries.txt` 同樣 tracked 且 `-diff`。自動內容約束是 8 項（R23 統一——R22 版這裡與測試各寫一個互斥的「唯一」；R24 版寫「6 項」並把數釘在
  `QueryFileReport` 的欄位數上，R25 指出那是**儲存屬性數**不是約束數；R26 加了第八條）。**權威與查法**：測試裡的
  `contentConstraints` 這份具名清單——每一項標明它經哪個出口回報、以及**哪一條測試的函式本體**必須斷言它；它自己
  寫明這套機制買到什麼、買不到什麼（三條同步檢查：散文的項數、每個 `QueryFileReport` 欄位都要被列到、每個出口都要
  在它**自己指名**的那條測試的函式本體裡有斷言，而第三條的 needle **從 outlet 導出**，R26——R25 版對 `throw` 那一項
  走的是一個寫死的原始碼字面、outlet 字串根本沒被讀。**買不到**的是「一條約束把違規記進既有欄位」，而那不是假設：
  檔首 BOM 已經是實例，被摺進「禁用字元」那一項）。實作與失敗史見 `checkQueryFile` 與 `contentConstraints`——
