...
- 查詢檔本身是一般 tracked blob、**明文在 GitHub 伺服器上**；`-diff` 只擋 diff 生成，規則 1 又禁止 review agent
  ...
  紀錄可查；`rrf-tie-queries.txt` 同樣 tracked 且 `-diff`。自動內容約束是 6 項（測試 `checkQueryFile`，`maxQueryScalars` 的說明是同一份；R23 統一——R22 版這裡與
  測試各寫一個互斥的「唯一」；R24：R23 版寫「封閉的四條」，照它自己指名的查法去數是六項——檔頭必備短語與條數下限沒被列，
  而後者就躺在原本第 (4) 條的括號裡）。**查法**：`checkQueryFile` 算出的 `QueryFileReport` 有幾個欄位就有幾項，真檔那條測試
  逐一斷言它們；散文的項數與逐欄位的斷言都由同步測試釘住（`contentConstraintCount`）。六項是：檔頭必備短語、條數下限、
  不得重複（與 judge 同形的正規化：空白摺疊＋小寫，R24——R23 版只比逐位元相等）、退役查詢的子字串比對（同一個正規化）、
  非註解行的每條純量上限（註解行沒有長度上限）、禁用字元（控制字元、非 ASCII 空白、BOM，含註解行）。另有一個不進
  `QueryFileReport` 的前置：非 UTF-8 直接 throw。它們擋不住整段文字（切成多條短行、或放進 `#`
  註解都過，codex R22），更分不出一句第三方逐字短句與自行撰寫的短語；作者自審是這個檔的防線。
