...
- 查詢檔本身是一般 tracked blob、**明文在 GitHub 伺服器上**；`-diff` 只擋 diff 生成，規則 1 又禁止 review agent
  ...
  紀錄可查；`rrf-tie-queries.txt` 同樣 tracked 且 `-diff`。自動內容約束是 7 項（R23 統一——R22 版這裡與測試各寫一個互斥的「唯一」；R24 版寫「6 項」並把數釘在
  `QueryFileReport` 的欄位數上，R25 指出那是**儲存屬性數**不是約束數，也因此把有自己驅動測試的「非 UTF-8 直接 throw」
  排除在外）。**權威與查法**：測試裡的 `contentConstraints` 這份具名清單，它自己寫明這套機制買到什麼、買不到什麼
  （三條同步檢查：散文的項數、每個欄位都要被列到、被列的欄位都要在真檔那條測試的**函式本體**裡有自己的斷言；
  **買不到**的是「一條新約束把違規記進既有欄位」——項數與欄位都沒變，三條全綠）。七項是：檔頭必備短語
  （必須在第一條查詢**之前**的連續註解區塊裡，R25——R24 版收的是全檔任意註解，檔頭整段搬到檔尾仍全綠）、
  條數下限（**只有下限、沒有上限**，R22；R24 改寫時掉了這句，R25 補回）、不得重複、退役查詢的子字串比對、
  非註解行的每條純量上限（註解行沒有長度上限）、禁用字元（控制字元、非 ASCII 空白、BOM，含註解行）、
  非 UTF-8 直接 throw。「不得重複」與「退役比對」共用一個正規化（空白摺疊成單一空白＋小寫，R24——R23 版只比
  逐位元相等），它**不與 judge 的 `norm()` 同形**，兩個方向都有：fold 本身較窄（`lowercased()` ≠ `casefold()`：
  ß／ς／相容分解字元），比對本身較寬（Swift String 的正則等價，NFC/NFD 兩種寫法判相等而 judge 判不等）——
  查法與後果寫在測試的 `foldLikeJudge` 說明裡（R25；R24 版這裡只寫「與 judge 同形」）。它們擋不住整段文字
  （切成多條短行、或放進 `#` 註解都過，codex R22），更分不出一句第三方逐字短句與自行撰寫的短語；作者自審是這個檔的防線。
