...
1. **查詢字串不得落進會被索引的欄位。** 具體（由上表導出，不是另一份清單；表改了這裡要跟）：
   ...
   **`grep '^#'` 之所以是安全操作，靠的是第 8 條自動內容約束**（第一條查詢之後不得有註解行）：沒有它，把一條查詢
   加 `#` 退役會讓那個字串脫離其餘每一條約束，而照這條規則執行 `grep '^#'` 的 reviewer 會把那條**仍然活著**的查詢
   原樣印進自己的逐字稿（R26，codex）。**明文退役必須刪除，不得以註解保留。**
   清「repo 之外的副本」要用**性質**不要用地點列舉：對每個受限檔，`find ~/.claude /private/tmp /tmp
   /private/var/folders -type f -name "$(basename $f)"`，排除本 repo 路徑與「git remote 指向本 repo 的 checkout」，
   其餘逐一 `cmp -s`。R25 收尾時用的是兩個地點的列舉（`git worktree list` 與 `/private/tmp` 的 glob），因此漏掉了
   `~/.claude/jobs/**` 裡七棵 reviewer 樹的明文複本、並把「repo 外零份」寫進了報告（R26，DA）。
...
- **規則在某個條目長度以下不可達成**——這一條決定的不是「有沒有漏洞」，是「這條規則對哪一半的受保護內容
  ...
  通過的測試輸出裡的次數。R26 量的結果：**基準查詢集（`baseline-queries.txt`）在整棵工作樹零命中**——#63 真正
  要建的那組儀器立得起來；而 sister set（`rrf-tie-queries.txt`）**超過一半**的條目逐字出現在工作樹裡，其中一批
  來自本 repo 自己的中文註解與 commit 文字，最短的條目只有兩個字元。規則 3 的 `self` 分診也救不了它：judge 的
  判定是子字串包含，所以那些命中每一個都會判成 `self`，而規則 3 的預設處置（退役）會退掉超過一半的集合。
  收進同一套紀律時（#68），要先決定 sister set 是換一組足夠長的條目、還是明寫它不受規則 1 保護。
...
- 查詢檔本身是一般 tracked blob、**明文在 GitHub 伺服器上**；`-diff` 只擋 diff 生成，規則 1 又禁止 review agent
  ...
  **這裡不複述項目內容**（R26：R23 版寫「封閉的四條」、R24 版「6 項」、R25 版又把七項逐一列在這裡，於是同一份列舉
  ...
  記憶體與子行程數無界（R26，codex；此檔只由作者撰寫、不接受外部輸入，所以記錄而不修）。它們擋不住整段文字
  （切成多條短行都過，codex R22），更分不出一句第三方逐字短句與自行撰寫的短語；作者自審是這個檔的防線。
