...
- verdict 是**封閉字母表**：`clean tool=<n>`／`self tool=<n>`／`empty tool=0`／
  ...
  斷言（只查帶「產生標記」且以 `#expect(` 開頭的活斷言行；標記字串在檢查函式裡是執行期拼出來的，
  所以那個函式自己的任何一行都不含它——R3 的版本回頭 grep 含答案的本檔永遠綠，R4 的版本把清單移走
  但標記字面仍在函式裡出現三次，R5 三方抓到；要驗的 token 從 `errorTokens` 導出）。
