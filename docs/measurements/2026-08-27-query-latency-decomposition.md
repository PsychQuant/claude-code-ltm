...
預測值是 −2.27 s（Python 量到的單次讀+雜湊），實測 −2.40 s。差額方向與「Swift 的
`hexDigest` 比 Python 的 OpenSSL 慢」一致，但**沒有對照量測**，所以那是推測。
...
**這不是「解決了 6 秒問題」**：3.79 s 仍然不是互動級延遲。移除的是可證明冗餘的那
一半；剩下的部分（其中約 2.3 s 是**必要**的續讀比對）需要換機制而不是省重複，
而那被本 repo 目前不存在的延遲目標值擋住。
