...
**兩個差值都要對帳，先前只對了較小的那個**（verify 抓到）：

| 條件 | 實測差 | 對預測 −2.27 的落差 |
|---|---|---|
| B. `query` | −2.40 | +0.13 |
| A. `build --quiet` | −3.02 | **+0.75**（近六倍）|

B 的 +0.13 方向與「Swift 的 `hexDigest` 比 Python 的 OpenSSL 慢」一致，但**沒有
對照量測**，所以那是推測。

**A 的 +0.75 沒有解釋，而且它讓帳算不平。** 診斷的模型是「掃描 6.50 s ＝ 兩次
讀+雜湊 4.51 ＋ 未歸屬約 2.0」，移除一次應得 `6.50 − 2.27 = 4.23 s`；實測
`A_post − D = 3.56 − 0.08 ≈ **3.48 s**`，比模型**還快 0.75 s**——等於未歸屬的那
2 秒自己縮水了三分之一，而本次沒有動過那段程式碼。候選（**都未查證**）：page
cache 狀態不同、記憶體壓力不同、或原本 6.50 s 那個 baseline 自己就偏高。

**所以修完之後仍有約 1.2 s 未歸屬**：

```
A_post − D ≈ 3.48 s
  ├─ 約 2.27 s  必要的續讀比對（讀+雜湊一次，省不掉）
  └─ 約 1.21 s  未歸屬
```

這個 1.2 s 先前在紀錄、commit message、Implementation Complete 三處都沒有出現。
查法就是上面那行減法。
...
**這不是「解決了 6 秒問題」**：3.79 s 仍然不是互動級延遲。移除的是可證明冗餘的
那一半；剩下的部分**其中**約 2.27 s 是必要的續讀比對（省不掉，只能換機制，而那
被本 repo 目前不存在的延遲目標值擋住），**另外約 1.2 s 仍然不知道花在哪**。
