...
|---|---|
| 掃描檔案數 | 8,744 |
| **會成為 chunk 的跨檔 turn**（＝本節要支撐的母體） | **2,736** |
| 其中時間戳相異者 | **0** |
| 相異率 | **0.00%** |
| （對照）只要求有 `uuid`+`timestamp` 的跨檔紀錄 | 23,908 |
| 其中時間戳相異者 | 0 |

**兩個母體差 8.7 倍，必須分清楚**——這是本節第一版的錯誤，由 #25 第三輪 verify 的
devils-advocate 抓到。初版只量了對照那列的寬鬆母體（23,908），但
`MAX(chunk_sources.timestamp)` 只在「一個 chunk 有 ≥2 個 source 列」時才有多於一個成員，
而那個母體是 **2,736**；23,908 裡絕大多數是 `tool_use` / `tool_result` / `summary` 這類
**永遠不會進索引**的行（上一節量到的 583,924 跳過行就是它們）。0 相異於超集確實蘊涵
0 於子集，所以結論沒變；但「引用的量測要涵蓋你正在宣稱的那件事」這條判準指的是**母體
也要對得上**——而本節存在的唯一理由正是上一次引用落空，初版等於在修法裡重犯同一類錯。

**與上一節的 12,488 不可直接比較**：那個數字套了 `uuid`+`sessionId`+`message` 三個欄位的
過濾，本節對照列的 23,908 只過濾 `uuid`+`timestamp`。兩者之間的差距來自過濾器不同，
不是語料在四天內長了那麼多。

**方法**：唯讀掃 `~/.claude/projects/**/*.jsonl`，逐行解析。
- 「會成為 chunk」＝ `type` ∈ {`user`,`assistant`} 且 `message.content` 取出的文字非空
  （與 `CorpusScanner` 的實際收錄條件對齊）。
- 跨檔 ＝ 該 `uuid` 出現在 ≥2 個不同檔案。
- 相異 ＝ 該 `uuid` 的 `timestamp` 集合大小 > 1。

**這份補量涵蓋什麼**：目前這份語料裡，resume 複製確實不改時間戳——在正確的母體
（2,736 個會成為 chunk 的跨檔 turn）上直接量到 0 相異。因此 `refreshNavigation` 的
`MAX(timestamp)` 目前退化為常數，不具備 `session_id` 那種「極值隨集合成長而移動」的
不穩定性。
