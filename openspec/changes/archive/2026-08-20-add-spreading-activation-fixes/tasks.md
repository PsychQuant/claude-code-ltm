...
- [x] 2.2 GREEN：`Sources/LTMMemory/Projection.swift` 的擴散迴圈（`for entry in deliberateContributions ... for other in members`）新增排除條件：`other` 若在 `suppression` 裡有非零值（即有自己的 `.dismissed` 事件）則跳過。2.1 轉綠，既有「擴散激發」段落全部測試維持通過。
