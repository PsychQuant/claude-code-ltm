...
- **Added 2026-08-20 (fix-round-2 of `add-spreading-activation-fixes`, found by a follow-up `/idd-verify #15`): the identical false AI4o provenance claim also appears in the Decisions section above (the `spreadingActivationFactor is a new, explicitly-unvalidated ProjectionParameters field` decision, "Default value carried over from `PsychQuant/ai4o`'s spreading-activation configuration…").** The first pass of this errata corrected only the Open Questions occurrence; this sentence closes the second one. Same correction applies: no such configuration was found, the shipped default is a self-chosen, unvalidated estimate.
