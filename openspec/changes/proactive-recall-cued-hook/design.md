...
- A recall block injected by the hook is stored as an `attachment` record, which the indexer never turns into a chunk (`attachmentRecordsAreNeverTurns`); it cannot become a query hit. A user turn that itself quotes the marker is indexed verbatim (there is no marker-stripping path).
- `ltm query --exclude-session S` never returns a hit whose `sessions` set is `{S}`; a hit with `sessions` `{S, T}` is still returned.
- `ltm query --max-refresh-seconds N` bounds the pre-query merge (scan + append batches) by roughly N — the deadline is computed at launch and covers the scan, so a scan that alone exceeds N merges zero batches — and skips rewritten sources entirely; total is ~N + retrieval, and the report names the unmerged source count. With a small all-append backlog the report shows zero unmerged.
...
- Marker constants: `RecallMarker.open = "<!-- ltm:recall v1 -->"`, `RecallMarker.close = "<!-- /ltm:recall -->"` — used by the `--format recall` writer only; the indexer does not read them (injected blocks are `attachment` records it already skips).
...
- **Bounded merge skips rewritten sources**: after a large session file is rewritten, a gated turn answers on the pre-rewrite content until an unbounded `ltm build` runs; the block reports the source as unmerged. Accepted: stale-but-present beats the live-lock the alternative caused (N1).

## Migration Plan

No index migration. The indexer is unchanged for existing turns (there is no marker-stripping path; injected recall lives in `attachment` records the scanner already skips). Plugin users receive the hooks on the next `claude plugin update ltm@claude-code-ltm`; the binary features ship in **v0.5.0**. **The plugin must not be published before v0.5.0 is released**: the wrapper pins the binary version, so a plugin update ahead of the binary leaves every gated turn printing one "version too old, need ≥ 0.5.0" line (once per session) and no recall — a deliberate, visible degradation, but only acceptable transiently. The hook checks the installed binary supports `--format recall` by grepping `ltm query --help` output before relying on it, otherwise it prints the notice naming the required version.
