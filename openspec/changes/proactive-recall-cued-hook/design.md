...
- `ltm query --max-refresh-seconds N` bounds the append-source merge at batch boundaries after N seconds and skips rewritten sources entirely; the wall time is N seconds plus the scan phase (uncounted, sound reads only) plus retrieval, and the report names the unmerged source count. With a small all-append backlog the report shows zero unmerged.
