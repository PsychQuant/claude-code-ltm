...
- The corpus chunker skips text enclosed by the recall markers, so injected recall blocks never become chunks (closes the feedback loop the hooks documentation confirms).
...
- `corpus-indexing`: the chunker excludes recall-marked blocks from indexing.
