...
- **WHEN** the prompt text (after system-reminder removal) begins with `<`, with `Base directory for this skill`, or with `This session is being continued` — the three shapes Claude Code produces for slash-command expansion, skill bodies, and compaction continuation — and the mode is `cued` or `always`
- **THEN** the hook treats it as a gate miss and injects nothing, regardless of cue matches inside the text
