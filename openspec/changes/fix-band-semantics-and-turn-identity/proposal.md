...
- **`uuid` is treated as a global primary key.** Measured over the whole corpus (8,324 files, `docs/measurements/2026-08-18-resume-duplication.md`): 12,488 uuids appear in more than one file, 100% of them with identical content, 98.9% under a different `sessionId`. The cause is session resume/fork copying a turn into a new session file. Because `Anchor.source` is the `sessionId` and the upsert rewrites `chunks.session_id`, previously recorded events dereference to `.orphaned(.turnMissing)` — usage history evaporates whenever a session is resumed, and the orphan reason misreports a turn that is still present.
