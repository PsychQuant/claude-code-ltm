...
An event SHALL consist of an event kind, an anchor, a timestamp, a generation identifier naming the index build that produced the result, a ranking policy identifier naming the strategy in force (or the reserved value described below when no single strategy was in force), and an optional presentation identifier naming the presentation the interaction originated in. A pin event SHALL additionally carry an opaque note reference generated as a random identifier. No event field SHALL contain corpus text, query text, or pin note text. A note reference SHALL NOT be derived from note content by hashing or by any other content-dependent function, and neither SHALL a presentation identifier.

**The ranking policy identifier has one reserved value, `interleaved`, and it names the absence of a single strategy rather than a strategy.** An interleaved comparison presents one ordering drawn from two strategies, so no single strategy was in force for it; the per-position attribution lives in the presentation record, not in the event. Writing either strategy's identifier onto those events would be a false statement about provenance, and writing nothing would make the events indistinguishable from single-strategy ones. The reserved value SHALL NOT appear among the identifiers a strategy can be constructed from — a reader who takes it for a strategy name and asks the registry for it gets nothing back, and the point of reserving it is that this mistake is refused rather than answered with silence (#36).
