...
`ltm build` SHALL run the indexing pipeline incrementally by default and from nothing when `--full` is given. All writes SHALL land under `~/.claude-ltm/`. On success it SHALL exit 0; on failure it SHALL exit non-zero with a message naming the reason naming the reason. **The reasons are exactly the cases of `IndexBuilder.BuildError` plus `CorpusScanner.ScanError` and the embedding provider's errors — this sentence deliberately does not restate them.** The previous wording listed five by name and fell two behind the code within one round (`stateUnreadable`, `lockUnsafe`); a list that claims to be complete and is not is worse than a pointer, because a reader who checks it finds it. The check a reader can rerun in thirty seconds: every case of those enums must have a branch in `BuildCommand`'s and `QueryCommand`'s `switch`, and Swift's exhaustiveness checking enforces that at compile time.
