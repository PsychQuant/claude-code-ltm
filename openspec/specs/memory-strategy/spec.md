...
**Ordering correctness is enforced at the seam, by twelve checks.** Ten of them run on every invocation. Two are conditional, and each states its own gate where it is listed: item 6 runs only for a strategy that declares it consumes history, and item 10 runs only for a strategy whose effective placement constraints are non-empty. Neither gate is a weakness — each exists because the check would otherwise reject correct behaviour — but a reader who assumes every listed check fires on every call will mis-diagnose the two that did not. "Conditional" here means gated on something the strategy declares about itself, and nothing else: on an empty candidate list most of these checks inspect nothing, but that is ordinary vacuity, it applies to items 4, 5, 8 and 11 alike, and it is not what items 6 and 10 are being distinguished by. They are enumerated here rather than summarised, because a summarising criterion acquires cases the enumeration never agreed to. They are listed in the order the entry point performs them:
...
4. Every candidate's base score is finite. Violation: `nonFiniteBaseScore`. This is a precondition rather than a formatting preference: a non-finite score makes equality comparison false against itself, and both the equal-segment split and the guard's permutation comparison are built on equality — an earlier revision hung in an infinite loop instead of failing.
5. Relevance bands are non-decreasing across the input order. Violation: `bandsOutOfOrder`. This is what makes the band fence meaningful: the guard verifies bands positionally, and positional comparison is equivalent to "no cross-band movement" only when each band occupies a single contiguous run of the input.
6. Every statistic in the projection is finite and non-negative. Violation: `malformedStatistics`. This is the one conditional check — it runs only when the strategy declares a non-empty set of consumed signals. It validates the whole projection rather than only the anchors in play, so that a malformed statistic surfaces here rather than at an unrelated query later.
7. The returned list is a permutation of the input candidates, compared as a multiset so that a duplicate cannot mask an omission. Violation: `candidateSetChanged`.
8. Each position's relevance band equals the band at that position in the input order. Violation: `crossedRelevanceBand`.
9. No candidate moves further than the displacement bound permits. Violation: `displacementBoundExceeded`.
10. Every placement constraint in the union of the authority table's entry and the strategy's own declaration holds. Violation: `movedAcrossTieRuns` for the tie-run constraint. This is the second conditional check: the union is empty for a strategy the authority table grants no constraints and which declares none, and an empty union means the check performs nothing. Of the three shipped strategies only one carries a tie-run constraint; for the other two this check is structurally inert, each for a stated reason recorded at its authority-table entry.
11. Each result's reported displacement, and separately its reported movement, equal the position change that actually occurred; and a reported history that claims counted signals, or claims the anchor is orphaned, matches what the projection actually supports. Violations: `misreportedDisplacement`, `misreportedMovement` and `misreportedHistory` — three checks rather than one, because a strategy can report an honest displacement beside a fabricated movement, and an honest movement beside a fabricated history.
    ...
    This is not a third conditional check in the sense of items 5 and 9. Those are gated on what the **strategy** declares about itself. This one is decided by the **result being examined**, which is the same shape as every other check in this list.

Each violation named above is raised by a `throw` reachable from the entry point that performs the checks — not from the checked method, which raises none of them, and which is itself public. That distinction is the whole of the gap named further below, so the enumeration's scope must be stated as the entry point that checks, never as "the public entry point": the protocol has two. Every such `throw` appears in this list. The correspondence is mechanically checkable and the check is stated so a reader need not reconstruct it: the thirteen names above SHALL match, one for one, both the cases of the `StrategyViolation` enumeration and the distinct violations thrown at the `throw StrategyViolation.` sites reachable from the public entry point.
