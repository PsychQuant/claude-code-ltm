...
**Ordering correctness is enforced at the seam, by eleven checks.** Nine of them run on every invocation. Two are conditional, and each states its own gate where it is listed: item 5 runs only for a strategy that declares it consumes history, and item 9 runs only for a strategy whose effective placement constraints are non-empty. Neither gate is a weakness — each exists because the check would otherwise reject correct behaviour — but a reader who assumes every listed check fires on every call will mis-diagnose the two that did not. They are enumerated here rather than summarised, because a summarising criterion acquires cases the enumeration never agreed to. They are listed in the order the entry point performs them:
...
Each violation named above is raised by a `throw` reachable from the seam's public entry point, and every such `throw` appears in this list. The correspondence is mechanically checkable and the check is stated so a reader need not reconstruct it: the twelve names above SHALL match, one for one, both the cases of the `StrategyViolation` enumeration and the distinct violations thrown at the `throw StrategyViolation.` sites reachable from the public entry point.
...
**The checks above run when the caller enters through the public entry point, and that entry point is not the only way in.** The checked method is itself a public protocol requirement. It takes a validated-candidates value whose initialiser is package-internal, so no outside caller can *manufacture* one — but that does not make the checked method unreachable, and this specification SHALL NOT be read as claiming it does. The value is public and sendable, and every conforming strategy is handed one on every call; a strategy that stores it, or passes it on, gives any code a key to call the checked method directly and skip every check enumerated above. A prior review demonstrated this rather than arguing it: a strategy returning three results for five candidates raised nothing.
...
Two questions therefore remain open rather than answered by this requirement: whether the checked method can be kept out of the public protocol surface without making the seam's violations untestable, and whether a strategy's reported identifier and displacement bound require an authority of their own. Both are tracked; neither is discharged here.
