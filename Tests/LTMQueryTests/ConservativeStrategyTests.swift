...
@Test func conservativeCannotMoveACandidateOutOfItsTieRun() throws {
    ...
    //
    // 現在 `checkTieRunsOnly` **只驗 tie-run 這一個維度**，位移上限由 seam 對
    // 所有策略無條件執行。這句話寫錯過兩次，紀錄留著：先前寫「這比位移上限
    // 更嚴格，不是它的豁免」——不成立，它只在「能否跨越語意區段」更嚴格，
    // 在幅度上完全不設限，兩者不可比較，所以那就是豁免（R4）；後來讓策略
    // 自己帶 bound 傳給守衛，而那個參數沒有任何測試釘得住（R5）。
    // 三次都錯在同一件事：**讓被約束者提供約束值**。
