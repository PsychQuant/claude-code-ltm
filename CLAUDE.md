...
regression 三個 lens 各自對著 `indexableText` 證偽了「印出」那一半（查法：issue #63 的 R1／R2
合併那則 verify comment，`issues/63#issuecomment-5563799192`——沒有單獨標題為 R1 的 comment——
Engine 段列了六個讀者、findings 段列了哪幾個抓到）——同一份 README
