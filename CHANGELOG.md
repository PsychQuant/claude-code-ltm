...
- **寫入站點檢查重寫：掃描抽成純函式，因為上一版的機制沒有執行點。**
  ...
  | `claimedMarkers` 以 `lastPathComponent` 為鍵，而 `Sources/` 有兩組重名檔 | basename | 鍵整個拿掉（每檔一份行號集合）|
- **收回 R14 對 #44 Expected ② 的收窄——它也是假的。** R14 說「兩個寫者照同一個
...
- **收回上一輪那道 `refuseSymlinkedMemoryRoot`——它裝錯了地方。** R12 診斷
  ...
  **而這一句在收回之後仍然原樣站了整整一輪，位置在同一個檔案、同一個
  `## Unreleased`、往下 16 行**——#44 R14 的頭號 CRITICAL 就是「新宣稱與同一個
