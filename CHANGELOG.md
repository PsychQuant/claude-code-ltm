...
- **基準查詢集與使用規則（#63）**：`scripts/baseline-queries.txt`（一行一條；檔頭以一行固定的終止符收尾，終止符之後
  ...
  （掛住而不是紅），淺 clone 則只會 fail open（見測試檔 `historicalQueryFolds` 的射程註解）；兩個例外見 `docs/measurements/README.md` 規則段；檔頭寫明列舉會漏、
  ...
  防禦內、哪些在外，權威在腳本檔頭「防禦邊界」段（R19：R18 版在這裡留了一份列舉、把邊界外的類別寫成擋得住）；白名單經繼承的 fd 3
  （有頭尾標記，沒到就 70）、argv 那一行有什麼、呼叫端控制哪些，以檔頭第 4 點為準、內容由測試釘到 `Sources/` 的環境變數讀取點；
  ...
  `2026-09-01-scan-parallelism.md` 補可比性註記（六條逐字記在紀錄裡、耗時可比、命中未記、新舊儀器不可對齊）；
...
- **#54/#55 的 verify 修正**：render 註解兩句可被單一輸入推翻的話收窄（「每輪
  非零」→ 一次量測＋樣本註明；「既有內容保留」→「若有既有索引則保留」）；
  `skipped` 排除補明示測試（兩個只差 SkipTally 的 outcome render 相同）；隱私
  斷言逐一化；#55 註解「每一個 sound 的偵測」收斂為「不存在完整、sound、又不進
  熱路徑的偵測」（codex 雙歷史構造：兩種來歷可產生逐 byte 相同現狀）。
...
- **收回一個被誤答的要求：#44 Expected ②「批次邊界上釋放寫鎖」從未實作。**
  `build()` **開頭**取得 process 層 `flock` 並全程持有，所以查詢路徑「插進來」
  的窗口是 **0%**（**只涵蓋索引工作**——第一個動作 `createRootIfNeeded()` 在鎖之外
  且本身是一個寫入；程式碼那一份已收窄，這一份 R16 漏掉，#44 R17 verify）；
  那把鎖來自 #24，**早於分批**，所以這條要求從來不可能靠分批
  本身滿足。CHANGELOG 先前寫的「並發寫入是具名拒絕（#44 ②）」**回答的是 issue
  裡另一句話**（Out of scope 段的「撞上寫鎖會怎樣尚未實測」）。
  這段話已經被改對過兩次、錯過兩次，兩次都是同一個形狀，所以連過程一起留著：

  1. R14 之前：「照字面實作會重新打開側車／DB 耦合」——**寫太寬**。
  2. R14 的更正：「兩個寫者照同一個協定（取鎖、讀 `scan_state`、寫一批、放鎖）
     走，耦合不會回來；真正的障礙是 `scan` 與 `vectorRow` **這兩個值**」
     ——**也是假的，而且與它自己的下一句矛盾**（若協定就夠，下一句那份工作量
     不存在）。#44 R15 verify 逐一證偽：

     - 那個協定沒有列 `vector_count`，而 `vectorRow` 是**本地計數器**、
       `appendVectors` 用 `seekToEnd()` 定位。重讀 `scan_state` 擋不住它。
     - 「障礙是這兩個值」是一份**宣稱自己完整的列舉**，漏了
       `truncateSidecar(toRows:)`——它整個 build 只跑一次，而**#44 的 issue body
       自己指名過這一項**。漏掉的那項是需求自己寫過的。

  現在成立的陳述與障礙**分類程序**寫在 `IndexBuilder.swift` 的 `build()` 開頭
  （「三個障礙」是舊版；那份清單後來變四項，再後來整個換成一條產生候選集合的程序）
  （查法：`grep -n "為什麼不現在補上" Sources/LTMIndex/IndexBuilder.swift`）。
  **那一份才是權威**——追 #53 的人落在程式碼，不落在這裡；上一輪只改了這裡而
  留下那一份，於是同一個 repo 對同一個問題給出兩個相反的答案整整一輪。
...
- **R17：上一輪的修法在兩處把方向搞反，而反在危險那一側。**
  R16 的 `open(` 排除用 **denylist**（「receiver 不是 libc 就排除」），於是
  `SomeModule.open(p, O_WRONLY)` 這種**任何**其他模組限定的寫入靜默逃掉
  （實測 0 flagged）。改成 **allowlist**（目前只有 `VectorSidecar`），未知的
  一律要求標記——**偽陽性看得到並修得掉，放行看不到**。
  同處還有一個短路：libc 那格直接 `return`，於是 `Darwin.open(p, O_RDONLY)`
  連唯讀排除都跑不到（1 flagged，而 `open(p, O_RDONLY)` 是 0）。
- **「指名是哪一個被抓」那條斷言是空的。** `unmarked()` 回報**整行**文字，所以
  同一行的兩個呼叫在輸出裡不可分辨——把排除反向（抓唯讀、放行寫入），
  `flagged.first?.contains("O_WRONLY")` **照樣是綠的**。現在回報帶欄位位置的
  **那個呼叫**，變異已確認為對的理由變紅。
- **誠實邊界第六次改寫：上一版說四條殘餘限度「都是偽陽性方向」——兩條方向標反了。**
  現在每一格都附合成輸入與實測的 flagged 數。
- **`#53` 的障礙清單第四次改寫：前三次都是「再列舉一次」。** 第三版把四個成員的
  符號名硬編成一條 regex 並自稱能「重算成員資格」——**一份封閉列舉換一種語法**，
  而且連它自己第 3 項都回傳不出來。換成一條機械程序（取鎖到迴圈之間的每一個
  繫結，逐一問「放鎖後還成立嗎」），候選集合由語法保證完整（22 個）。
  照這條程序找到 `grouped`：放鎖後另一個寫者提交過，第一個寫者仍用舊的
  `grouped[X]` 再 insert 一次；而既有的補救只對失效來源觸發，**純 append 的來源
  沒有 delete 蓋過去**——重複 chunk、無 error path、違反不變式 2。
  它同樣是 **#44 issue body 自己寫出來的**。
- **行距數字全部拆掉，不是再算一次。** 上一版新加的「相隔 49 行」「往下 32 行」
  三重錯：數字對不上任何一個狀態、`grep` 會命中註記自己、而**行距在一個每輪增行的
  檔案裡是構造上的移動標的**——同一次 commit 另一處剛把 `..HEAD` 判為移動標的。
- 其餘副本：`三種佈局` 的第三份、`三個障礙` 的兩份、`窗口 0%` 的收窄、
  `sourceFiles` 相對路徑的 `/var` vs `/private/var` 深度差、`lock-open` 表列措辭。
- **R16：上一輪的四個新機制裡有一個仍然沒有執行點，另一個仍然是逐行的。**
  抽成純函式的只有 `markers` / `unmarked` / `writeCallOffsets`——**新增的名稱唯一性
  檢查留在 `@Test` 裡、只跑真實的樹**，而樹裡零重名，所以把判準改成 `> 99` 也全綠。
  已抽成 `WriteSiteScan.duplicateNames(in:)` 並由合成輸入驅動。

  同輪另外三個：`FileHandle.standard` 的排除從整行縮成 **60 字元回看**（同一個缺陷
  換一個參數——實測藏不藏取決於**空白字元的數量**），已改成 **receiver 定界**；
  `Darwin.open(` 被當成 Swift 方法名靜默排除，已加 libc 限定詞例外；`argumentText`
  在括號不閉合時取到行尾、吞掉**後面另一個呼叫**的 `O_RDONLY`，已改成回 `nil`
  （無法判定時不排除）。四者各有一條具名測試，逐一退掉各自變紅。

  另外刪掉一個死守衛（`func openDerivedFileNoFollow` 那條——退掉它零測試變紅，
  第 1 條排除已涵蓋），並把「逐一退掉任一機制都會變紅」這句**假的全稱**換成量出來的
  分工表（五個實測反例）。
- **誠實邊界第五次改寫：上一版宣稱的一個「實測過」的逃逸不存在。** 原文說
  `try data\n    .write(to: …)` 逃得掉——**三種拆法全部抓得到**。現在這一節只寫量過的，
  四條排除規則各自的殘餘限度逐條列出並各附合成輸入。
- **寫入站點檢查重寫：掃描抽成純函式，因為上一版的機制沒有執行點。**
  R14 加的 `claimedMarkers` 退掉之後 516 條測試全綠——它跑在真實的樹上，而樹裡
  按定義不含它要防的反例。本 repo 對這種東西的處置是**刪掉**，而我在同一輪用這條
  規則刪過一個 `O_NOFOLLOW`。這裡不刪而是給它執行點（`WriteSiteScan` + 七條合成
  輸入測試），因為它防的缺陷真實存在。**五個機制逐一退掉，各有測試變紅。**

  同時關掉三個逃逸（「宣稱的範圍比實際執行的寬一級」）與一個隱患。
  **上一版把四列都寫成「逃逸」——第四列不是**：`lastPathComponent` 碰撞在當時
  不可達（重名檔一個標記都沒有），而且沒有任何測試驅動它，它是預防性的
  簡化（#44 R16 verify）：

  | 逃逸 | 上一版 | 現在 |
  |---|---|---|
  | 同一行上的兩個呼叫共用一個標記（含折成一行的三元運算子）| 逐**行**認領 | 逐**呼叫** |
  | 複製既有標記**名稱**就隱形（兩邊都是 `Set`，重名被去重吃掉）| 無檢查 | 名稱唯一性檢查 |
  | 同一行出現 `O_RDONLY` 就整行跳過，真正的寫入一起隱形 | 逐**行**排除 | 逐**比對**（括號配對取引數）|
  | （**隱患，非逃逸**）`claimedMarkers` 以 `lastPathComponent` 為鍵，而 `Sources/` 有兩組重名檔——當時不可達 | basename | 鍵整個拿掉（每檔一份行號集合）|
- **收回 R14 對 #44 Expected ② 的收窄——它也是假的。**
  （**這一則與上方 `## Unreleased` 開頭那則講同一件事**——同一次 commit 寫了兩份，
  於是下一輪要同步的副本從一份變成兩份，#44 R16 verify。權威的敘述在
  `IndexBuilder.swift` 的 `build()` 開頭；這兩則都只是指標。

  **上一版在這裡寫「相隔 49 行」並附了一條「兩個行號相減」的查法。那是三重錯的**
  （#44 R17 verify）：數字對不上改動前（46）也對不上改動後（66）；`grep` 會命中
  註記自己所以「相減」在字面上未定義；而且**行距在一個每輪都在增行的檔案裡是
  構造上的移動標的**——同一次 commit 另一處剛把 `..HEAD` 判為移動標的並釘死 ref。
  **所以行距整個拆掉，不是再算一次。** 要找另一則就 grep 它的標題。）R14 說「兩個寫者照同一個
  協定（取鎖、讀 `scan_state`、寫一批、放鎖）走，耦合不會回來；障礙是
  `scan` 與 `vectorRow` 這兩個值」。兩句都不成立：那個協定沒有列 `vector_count`
  （而 `appendVectors` 用 `seekToEnd()` 定位），而「這兩個值」是一份宣稱自己完整
  的列舉，漏了 `truncateSidecar(toRows:)`——**#44 的 issue body 自己指名過它**。
  現行陳述與障礙**分類程序**寫在 `IndexBuilder.swift` 的 `build()` 開頭，那一份才是權威。
  （「三個障礙」是舊版數字，見上一則。）
- **`man 2 open` 那句引文不在 `man 2 open` 裡。** R14 寫「**這一句是可查的**：
  `man 2 open`「The mode argument … is used only when a new file is created」」
  ——Darwin 的 man page 沒有這句（那是 Linux 措辭），它只寫了反向。**行為宣稱
  本身是對的**（已實測），假的是那個自稱可查的引用。連帶更正「少給第三個引數
  mode 取自堆疊上的垃圾」——實測是確定性地傳 0。
- **`ltm mark` 的不變式 1 現在有兩條測試，分工由變異量出來。** 舊那條的檔案系統
  斷言在它自己的佈局下**不可能紅**（symlink 指向既存目錄，`mkdir -p` 是 no-op）。
  新增一條指向語料內**尚不存在**路徑的測試：拿掉 `LTMService.make` 的 containment
  → 新測試的兩個檔案系統斷言紅、舊測試只有訊息斷言紅。
- **收回「不變式 1 在記憶層這一側是縱深防禦」那句話——反例不需要 symlink。**
  ...
  **而 `LTMService.swift` 那段程式碼上方三行的註解已經這樣寫了**（「目錄那時
  ...
  `mkdir -p` 對它是 no-op），而那一格恰好是唯一支持我結論的。
  表的判定已改回**一層**。

  （上一版這裡寫「三種佈局裡唯一支持我結論的一格」，卻只指名了兩種、也沒有可
  指名的量測——**又一份宣稱自己完整的列舉**，#44 R15 verify。實際被量過的只有
  兩格：連到既存目錄（no-op、看不到寫入）與指向不存在的路徑（兩個目錄被建出來）。
  「共有幾種」我沒有查過，所以不寫。）
- **備份那個 `O_NOFOLLOW` 已刪除。** 上一版留著它「讓意圖顯式」——而那正是本 repo
  那條規則明文禁止的處置，**且我在同一輪用同一條規則刪掉了另一道守衛**。
  同一條規則、同一輪、相反的兩個處置。
- ~~**寫入站點檢查：一個標記只背書一個呼叫。**~~ **（R15 收窄：實作是逐**行**認領，
  同一行上的兩個呼叫仍然共用一個標記；R16 再收窄：`FileHandle.standard` 的排除仍是
  字元窗口。現行敘述見上方「寫入站點檢查重寫」那則——同一個 `## Unreleased` 裡對同
  一個機制的兩則描述，舊那則未加註站了一輪，#44 R16 verify。）** 三行視窗讓相鄰的第二個站點借用前一個
...
- **收回上一輪那道 `refuseSymlinkedMemoryRoot`——它裝錯了地方。** R12 診斷
  ...
  **零**：**兩種**佈局實測顯示（「三種」是沒查過就寫下的數量詞，#44 R17 verify；
  被量過的只有兩格，見上方 `## Unreleased` 裡的佈局表），指進語料樹的那一格由更早的
  ...
  只是訊息退化。~~ **（已於上方收回——反例不需要 symlink：指向語料裡一條尚不
  存在的路徑時，兩個目錄先被寫進唯讀語料，第二層才拒絕。**

  **而這一句在收回之後仍然原樣站了整整一輪，位置在同一個檔案、同一個
  `## Unreleased` 之內**（上一版寫「往下 32 行」並附「兩個行號相減」的查法——
  **32 在寫下它的前一刻是對的，在註記自己被插進檔案的那一刻變假**，因為註記加了
  三行。行距在這個檔案裡是移動標的，已整個拆掉，#44 R17 verify）——#44 R14 的頭號 CRITICAL 就是「新宣稱與同一個
  檔案裡三行外的既有註解矛盾」，而 R14 的修法逐字複製了那個形狀，只是這次矛盾
  的兩端都在 `CHANGELOG.md` 裡（#44 R15 verify，requirements + regression）。
  查法：`grep -n 縱深防禦 CHANGELOG.md` 回三行——收回的標題、被刪除線包住的
  原句、以及這條查法自己。**沒有第四行、也沒有任何一行是未加註的原宣稱。**
  （上一版這裡寫「必須只回這一處」——**我沒有跑就寫下它，而它回的是三行**。
  同一段裡，在一句規定查法的話上再犯一次。））**
  新測試扛的是「上游那一層還在」，不是「不變式沒被違反」——這個區分寫在測試裡。
...
- **那張寫入站點表現在真的被檢查了。** 上一版的檢查掃 grep pattern 數出一個數字、
  ...
  **我調常數調到綠，然後編了一個解釋。**
  ...
  第一欄雙向比對，(b) 每一個寫入 primitive 的呼叫上方 **3 行內**必須有標記。
  （這一句原本寫「兩行內」。R15 修掉了 `WriteSiteTableSyncTests.swift` 裡的那一份、
  漏了這一份，而 R15 報告把該項列在「已全數處理」——#44 R16 verify。
  查法：`grep -rn 兩行內 --include='*.swift' --include='*.md' .` 回**兩行，都在本則
  註記自己裡面**——沒有第三行，也沒有任何一行是未加註的機制描述。
  （上一版寫「應回零」，而註記文字自己就含那三個字。**同一輪、同一種踩法的第二次**
  ——把查法寫進註解之後 grep 會數到查法自己，我在 `IndexBuilder.swift` 為此加了
  `grep -v '//'`，卻沒對 markdown 套用同一個教訓。）
...
- **查詢撞上建置鎖時說出來**，不再靜默降級。查詢仍以既有索引回答（拒答更糟），但
  ...
  （**這句話寫下時只對 CLI 門面成立**——`RefreshReport` 的旗標在 MCP 門面零消費，
  Claude 經 MCP 查記憶時拿不到任何訊號，整整站了一輪才被 #51 抓到。現已兩個門面
  消費同一個旗標。另一半界線也在 #51 說清楚：降級只涵蓋 `lockHeld`（會自癒）；
  `lockUnavailable`（權限／磁碟滿，不修不會好）刻意照樣硬失敗——否則查詢會永遠
  跑在舊索引上而每輪都「成功」。）
