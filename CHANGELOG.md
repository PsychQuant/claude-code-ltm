...
### Added
- **#18：RRF 平手發生率已量**（`docs/measurements/2026-08-22-rrf-tie-rate.md`、
  探針 `scripts/measure-rrf-ties.swift`、查詢集 `scripts/rrf-tie-queries.txt`）。
  `conservative` 在 base score 兩兩相異時可證明等同於 `archival`，所以平手發生率
  就是這一檔的全部價值——先前沒量過，而 code 註解一度宣稱它「實務上會命中」。
  - **結論：不是佔位符，但作用面極度集中在 `cjk-2char`。** 16,959 chunk 的索引上，
    中文雙字查詢的**每一個** top-20 候選都落在平手段內（相鄰平手 52.6%）；
    `cjk-4plus` 與 `mixed` 一次都沒觀測到平手，那兩桶裡它逐字等同 `archival`。
    整體 12.9% 會把這種集中洗掉——分桶報告在這裡再次證明是必要的。
  - **52.6% 是結構上限不是觀測噪音**：k=20 且平手成對出現 → 10 對／19 個相鄰配對
    = 10/19。大語料量到的正是這個數，所以該桶已觸頂，**全語料未觀測這個缺口對它
    是有界的**（其餘四桶沒有這個保證）。
  - **機制是 `trigram` 3 字元硬底線的下游後果**：平手需要兩個通道各自貢獻獨佔候選
    並在名次上對撞。中文雙字查詢的 trigram 通道整條空掉（`2026-08-10-fts5-tokenizer.md`
    已記錄），只剩 `segment` 與 `vector` 兩組互斥的單通道候選成對同分——這也解釋了
    實測段長**恆為 2**。同一條硬底線既壓低了中文雙字的召回，也製造了這一檔的作用面。
  - **量的是生產路徑**：探針不含任何檢索邏輯，呼叫 `ltm query --json` 讀回真正的
    `score`／`band`。在探針裡重算一次 RRF 會量到那份重寫的平手率（「兩個寫者＝兩份
    會漂移的規格」）。JSON 的 Double 序列化已實測在 1 ulp 粒度單射，所以精確相等
    比較是忠實的。分桶用生產的 `QueryClassifier`（編譯時帶入該原始檔）。
  - **探針驗過會動**：餵一組手算過答案的固定輸入，八個輸出欄位全中，其中
    80%（僅 score）vs 60%（band+score）的差距證明兩個判準真的被分別計算。
  - 踩到一個**不會報錯**的誤設：只設 `LTM_DERIVED_ROOT` 而未設 `LTM_CORPUS_ROOT` 時，
    每次查詢都對真實語料做增量續讀（單一查詢燒掉三分鐘 CPU 仍未返回），量到的索引
    不是指定的那個而且中途還在長大。探針現在對此拒跑並說明理由。
