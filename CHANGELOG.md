...
- **收回一個被誤答的要求：#44 Expected ②「批次邊界上釋放寫鎖」從未實作。**
  ...
  現在成立的陳述與三個障礙寫在 `IndexBuilder.swift` 的 `build()` 開頭
...
- **收回 R14 對 #44 Expected ② 的收窄——它也是假的。**
  （**這一則與上方 `## Unreleased` 開頭那則講同一件事，相隔 49 行**——查法：
  `grep -n "收回一個被誤答的要求" CHANGELOG.md` 與 `grep -n "收回 R14 對 #44 Expected" CHANGELOG.md`
  兩個行號相減。 同一次
  commit 寫了兩份，於是下一輪要同步的副本從一份變成兩份——正是本檔往下 45 行處
  自己在譴責的形狀，#44 R16 verify。權威的敘述在 `IndexBuilder.swift` 的
  `build()` 開頭；這兩則都只是指標。）R14 說「兩個寫者照同一個
  ...
  現行陳述與三個障礙寫在 `IndexBuilder.swift` 的 `build()` 開頭，那一份才是權威。
...
- **收回上一輪那道 `refuseSymlinkedMemoryRoot`——它裝錯了地方。** R12 診斷
  ...
  **而這一句在收回之後仍然原樣站了整整一輪，位置在同一個檔案、同一個
  `## Unreleased`、往下 32 行**（查法：`grep -n "收回「不變式 1" CHANGELOG.md` 與
  `grep -n "同一輪的變異另外揭露" CHANGELOG.md` 兩個行號相減；上一版寫「16 行」而
  **沒有附查法**，出現在一段以「我沒有跑就寫下它」作結的文字裡——#44 R16 verify）——#44 R14 的頭號 CRITICAL 就是「新宣稱與同一個
