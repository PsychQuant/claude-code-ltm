...
- **預設不寫使用歷史**。`--record` 才會寫 `shown` 事件——開發與檢查用的查詢會污染
  策略比較所依據的資料。**讀歷史不需要 `--record`**：`--strategy human-like` 會讀
  既有歷史來排序，但不寫入任何東西。
- **索引過期時拒答而不降級**。三種情況都拒答並指名 `ltm build --full`：embedding
  revision 不同代、anchor 定址規則不同代、向量側車檔與索引宣稱的筆數對不上。
  共同的理由是這些失敗**看起來都很正常**——跨代的向量距離不會報錯，少一路通道的
  結果也不會，使用者只會覺得「這東西找不到東西」。
