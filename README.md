...
**手動安裝**（不裝 plugin 時）三步。這三行實跑驗證過：

```bash
# 1. 拿 binary。Developer ID 簽章 + notarized，Gatekeeper 直接放行
curl -fL https://github.com/PsychQuant/claude-code-ltm/releases/download/v0.5.0/ltm -o ~/bin/ltm
chmod +x ~/bin/ltm
...
```bash
curl -fsL https://github.com/PsychQuant/claude-code-ltm/releases/download/v0.5.0/ltm.sha256
